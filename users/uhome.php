<?php
// uhome.php
// Dashboard page that loads the Sale Tracker (sale.php) into a modal via embed mode.
// Update DB credentials in sale.php as needed.
...
<head>
  ...
  <style>
    ...
    .metric-title { font-size: 0.95rem; color: var(--muted); font-weight: 600; }
    .metric-value { font-size: 2.4rem; font-weight: 700; color: #0f62fe; margin-top: 6px; }
    .metric-sub { color: #8a8f98; font-size: 0.85rem; margin-bottom: 12px; }
    .field-label { font-size: 0.82rem; color: #6b7280; margin-bottom: 6px; display:block; font-weight:600; }
    .field-input { width:100%; padding: 8px 10px; border-radius: 8px; border: 1px solid #e6e9ef; background: #fbfdff; font-size: 0.9rem; }
    .activity-bar { margin-top: 18px; padding: 12px 16px; border-radius: 8px; background: linear-gradient(90deg, rgba(15,98,254,0.12), rgba(0,184,148,0.08)); display:flex; justify-content:space-between; align-items:center; color:#fff; font-weight:600; }
    @media (max-width: 767.98px) {
      .metric-value { font-size: 1.8rem; }
      .app-container { padding: 16px; margin: 16px; }
    }
    .spinner-center { display:flex; align-items:center; justify-content:center; min-height:160px; }
  </style>
</head>
<body>
  ...
  <nav class="navbar navbar-expand-lg topbar">
    <div class="container-fluid">
      <a class="navbar-brand d-flex align-items-center gap-2" href="#">
        ...
        <div class="brand-title ms-2">UHome</div>
      </a>
      ...
      <div class="collapse navbar-collapse" id="topNav">
        <ul class="navbar-nav ms-auto align-items-center">
          <li class="nav-item me-3 d-flex align-items-center">
            ...
            <a class="nav-link text-white p-0" href="#">Dashboard</a>
          </li>
          <li class="nav-item me-3 d-flex align-items-center">
            <i class="bi bi-bar-chart-line nav-icon me-2"></i>
            <a class="nav-link text-white p-0" href="#">Sales Tracker</a>
          </li>
          <li class="nav-item me-3 d-flex align-items-center">
            <i class="bi bi-wallet2 nav-icon me-2"></i>
            <a class="nav-link text-white p-0" href="#" id="openSaleTracker">Sale Tracker</a>
          </li>
          <li class="nav-item me-3 d-flex align-items-center">
            <i class="bi bi-shop nav-icon me-2"></i>
            <a class="nav-link text-white p-0" href="#">Store Performance</a>
          </li>
        </ul>
      </div>
    </div>
  ...
  <!-- Sale Tracker Modal (loads sale.php?embed=1 into modal body) -->
  <div class="modal fade" id="saleTrackerModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Sale Tracker</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div id="saleModalBody" class="modal-body">
  ...
  <script>
    // Click handler: fetch sale.php in embed mode and inject into modal body
    document.getElementById('openSaleTracker').addEventListener('click', function (e) {
      e.preventDefault();
      const modalEl = document.getElementById('saleTrackerModal');
      const modal = new bootstrap.Modal(modalEl);
      const body = document.getElementById('saleModalBody');

      // show loader and modal
      body.innerHTML = '<div class="spinner-center"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div></div>';
      modal.show();

      fetch('sales.php?embed=1')
        .then(resp => {
          if (!resp.ok) throw new Error('Network response was not ok');
          return resp.text();
        })
        .then(html => {
          // Inject fragment. The fragment includes an inline script that calls initSaleEmbedded()
          body.innerHTML = html;
          // initSaleEmbedded will be called by the embedded script automatically
        })
        .catch(err => {
          body.innerHTML = '<div class="p-3 text-danger">Could not load Sale Tracker. Ensure <code>sale.php</code> is available.</div>';
          console.error(err);
        });
    });

    // small visual focus on expenses profit
